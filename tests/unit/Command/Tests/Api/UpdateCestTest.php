...
final class UpdateCestTest extends TestCase
{
    public function testExecuteDryRun()
    {
        $writer = $this->prophesize(WriteInterface::class);
        $tester = new CommandTester(new UpdateTestCommand(null, $writer->reveal()));
        $tester->execute([
            'entity' => Hubby::class,
        ]);
        ...
        $this->assertContains('UpdateCest', $display);
    }
    ...
    public function testExecuteForce()
    {
        ...
        $tester = new CommandTester(new UpdateTestCommand(null, $writer));
        $tester->execute([
            'entity' => Hubby::class,
            '--force' => true,
        ]);
        ...
        $this->assertContains('UpdateCest', $content);
    }
}
